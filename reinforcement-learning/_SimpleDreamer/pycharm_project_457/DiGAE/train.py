...
from preprocessing import general_train_test_split_edges, \
    biased_train_test_split_edges, bidirectional_train_test_split_edges
...
for i in range(args.nb_run):
    ...
    if feature_vector_type in ['svd', 'svd_randomized', 'random', 'normal',
                               'random_ones']:
    ...
    if args.model == 'digae':
        encoder = DirectedGCNConvEncoder(in_channels, hidden_channels,
                                         out_channels, alpha=args.alpha,
                                         beta=args.beta,
        ...
        encoder = SingleLayerDirectedGCNConvEncoder(in_channels, out_channels,
                                                    alpha=args.alpha,
                                                    beta=args.beta,
                                                    self_loops=args.self_loops,
                                                    adaptive=args.adaptive)
