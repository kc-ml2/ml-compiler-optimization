...
from dreamer.modules.model import RSSM, RewardModel, ContinueModel, RuntimeModel
...
class Dreamer(nn.Module):
    def __init__(
        ...
        self.runtime_predictor = RuntimeModel(config).to(self.device)
        ...
        self.model_params = (
            ...
            + list(self.runtime_predictor.parameters())
        )
    ...
    def train(self, env):
        ...
        for iteration in range(self.config.train_iterations):
            ...
            #     torch.save(
            #         self,
            #         f"./models/{datetime.now().strftime('%Y%m%d-%H%M%S')}dreamer_{iteration}.pt"
            #     )
    ...
    def _model_update(self, data, posterior_info):
        ...
        runtime_dist = self.runtime_predictor(
            posterior_info.posteriors, posterior_info.deterministics
        )
        runtime_loss = runtime_dist.log_prob(data.runtime[:, 1:])
    ...
    def environment_interaction(self, env, num_interaction_episodes, train=True):
        for epi in range(num_interaction_episodes):
            ...
            while not done:
                ...
                next_observation, reward, done, info = env.step(env_action.item())
                if train:
                    self.buffer.add(
                        observation, buffer_action, reward, next_observation, done, env.state.walltime
                    )
                score += reward
                embedded_observation = self.encoder(
