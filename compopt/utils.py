...
#         max_len = 1000
#         node_space = Tuple([
#             Discrete(3),  # node type
#             Discrete(7696)  # token
#         ])
#         edge_index_space = Tuple([
#             Discrete(max_len),  # node type
#             Discrete(max_len)  # token
#         ])
#         edge_space = Tuple([
#             Discrete(3),  # flow
#             Discrete(19)  # position
#         ])
#         self.observation_space = Tuple([
#             _Repeated(node_space, max_len=max_len),
#             _Repeated(edge_index_space, max_len=max_len ** 2),
#             _Repeated(edge_space, max_len=max_len ** 2),
#         ])
#
#     def reset(self, **kwargs):
#         obs = super(Preprocess, self).reset(**kwargs)
#
#         return preprocess(obs)
#
#     def step(self, ac):
#         obs, rew, done, info = super().step(ac)
#
#         return preprocess(obs), rew, done, info
