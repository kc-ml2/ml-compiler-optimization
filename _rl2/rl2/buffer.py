...
class DequeBuffer:
    def __init__(self, maxlen=int(1e6)):
        self.obs = deque(maxlen=maxlen + 1)
        self.action = deque(maxlen=maxlen + 1)
        self.reward = deque(maxlen=maxlen + 1)
        self.done = deque(maxlen=maxlen + 1)
        self.value = deque(maxlen=maxlen + 1)
        self.log_prob = deque(maxlen=maxlen + 1)

    def __len__(self):
        return len(self.obs)

    def append(self, obs, action, reward, done, value, log_prob):
        self.obs.append(obs)
...
class NumpyBuffer(np.ndarray):
    ...
    def __new__(
        ...
        assert (spec is not None) or (example is not None), \
            f'you must provide one of following arguments\n\tspec, example'

        if not all([k in example.keys() for k in NumpyBuffer.default_keys]):
            warnings.warn(
                f'your example missing one of keys\n\t'
                f'{NumpyBuffer.default_keys}')
